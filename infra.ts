...
export const searchUser = async (user: string) => {
	...
	const nsoutput =
		nsroles.length > 0
			? `${user} has the following roles for this namespace (${currentNS}): ${nsroles}`
			: `${user} has no roles for this namespace`;
	const clusteroutput =
		clusterroles.length > 0
			? `${user} has the following roles for this cluster: ${clusterroles}`
			: `${user} has no roles for this cluster`;
	output = `${nsoutput}\n${clusteroutput}`;
	return output;
};
...
export const searchGroup = async (group: string) => {
	...
	let clustergrants = "";
	const u = Deno.run({
		cmd: ["infra", "groups", "list", "--no-truncate"],
		stdout: "piped",
	});
	const g = Deno.run({
		cmd: ["infra", "grants", "list", "--group", group],
		stdout: "piped",
	});
	const uout = new TextDecoder()
		.decode(await u.output())
		.trim()
		.split("\n")
		.slice(1)
		.map((line) => line.trim().split(/\s{2,}/g))
		.filter((line) => line[0] === group)[0][1]
		.split(", ");
	new TextDecoder()
		.decode(await g.output())
		.trim()
		.split("\n")
		.slice(1)
		.map((line) => line.trim().split(/\s{2,}/g))
		.map((line) => {
			if (line[2] === `${dest}.${ns}`) {
				nsgrants +=
					nsgrants.length === 0
						? `Group ${group} has the following roles on this namespace (${ns}): ${line[1]} `
						: `, ${line[1]}`;
			} else if (line[2] === dest) {
				clustergrants +=
					clustergrants.length === 0
						? `Group ${group} has the following roles across the cluster: ${line[1]} `
						: `, ${line[1]}`;
			}
		});
	const users =
		uout[0] !== "0"
			? `Users in group ${group}: ${uout.join(", ")}`
			: `No users in group ${group}`;
	const nsout =
		nsgrants.length > 0
			? nsgrants
			: `Group ${group} has no grants on this namespace (${ns})`;
	const clusterout =
		clustergrants.length > 0
			? clustergrants
			: `Group ${group} has no grants on this cluster`;
	const grants = `${nsout}\n${clusterout}`;
...
const isGroup = async (test: string) => {
	...
	if (cache.includes(`${test}isGroup`)) {
		...
		if (groupcache.length === 0) {
			const p = Deno.run({
				cmd: ["infra", "groups", "list"],
				stdout: "piped",
			});
			poutput = new TextDecoder()
				.decode(await p.output())
				.trim()
				.split("\n")
				.splice(1)
				.map((val) => val.trim().split(/\s{2,}/)[0]);
			groupcache = poutput;
		}
	...
	if (cache.includes(`${test}isRole`)) {
		...
		if (rolecache.length === 0) {
			...
			poutput = new TextDecoder()
				...
				.concat(["cluster-admin", "admin", "edit", "view"]);
			rolecache = poutput;
		}
...
export const addUserGroup = (user: string, group: string) => {
	return `user ${user} should be added to group ${group}`;
};
